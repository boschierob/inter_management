...
def run_real_delivery():
    ...
    for msg in messages:
        ...
        mail = Mail(
            ...
            to_emails=destinataires,  # Utilise la liste complète
            subject=sujet,
        ...
        if has_attachment:
            try:
                ...
                # On affiche la liste des destinataires pour le log
                print(f"🟡 Envoi en cours à : {', '.join(destinataires)}")

                # Mise à jour du statut dans Notion
            ...
            print(
                f"⚠️ Message {msg_id} ignoré "
                f"(aucune facture unpaid avec lien Supabase trouvée)"
            )
